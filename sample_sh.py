...
object_center = calculate_center(xyzs_object)
distances = calculate_dist_vectorized(object_center, xyzs_background)
# print(distances.min(), distances.max())
sorted_indices = torch.argsort(distances)
sorted_bg_gaussians_xyzs = xyzs_background[sorted_indices]
sorted_distances = distances[sorted_indices].tolist()
# print(sorted_distances[0], sorted_distances[-1])


# use sorted indices to sample bg SH
selected_indices = sorted_indices[:len(xyzs_object)]
selected_gaussians_dc = background_gaussians['features_dc'][selected_indices]
selected_gaussians_rest = background_gaussians['features_rest'][selected_indices]
print('Nearest gaussians SH sampled from the background')

# switch SH
object_gaussians['features_dc'] = selected_gaussians_dc
# object_gaussians['features_rest'] = selected_gaussians_rest
print('Switch SH finished, begin merging...')
